...
const Project: FC<IProps> = ({ project }, index) => {
  ...
  return (
    ...
    >
      <figure className="relative w-full p-md:w-7/12 h-max p-md:aspect-video shadow-inner shadow-nord0 dark:shadow-transparent">
      ...
      <div className="w-full p-lg:w-5/12 h-max mt-10 p-lg:mt-0">
        ...
        >
          <figure className="grid grid-cols-2 p-sm:grid-cols-3 p-xl:!grid-cols-2 grid-rows-2 p-sm:grid-rows-1 p-xl:!grid-rows-2 p-xl:gap-x-16 gap-y-10 p-sm:gap-y-0 p-xl:!gap-y-10 w-full p-xl:w-1/2 mb-10 p-xl:mb-0">
            {project
              ? project.tools.map(renderTools)
              : emptyTools.map(renderTools)}
          </figure>
          <nav className="flex flex-col gap-y-5 w-full p-xl:w-1/2">
            <ProjectLink
              url={
                project ? project.githubUrl : "https://github.com/emrhnpla02"
              }
            >
